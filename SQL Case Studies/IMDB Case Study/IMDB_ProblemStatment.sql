...
WITH cte AS (
SELECT i.title, i.movie_id, (e.domestic + e.worldwide - i.budget) AS net_profit
FROM movies.earning e
JOIN movies.imdb i
ON i.movie_id = e.movie_id)
...
WITH cte AS (
SELECT g.genre AS genre, (i.rating + i.metacritic / 10.0) AS weighted_rating
FROM movies.imdb i
JOIN movies.genre g
ON i.movie_id = g.movie_id
