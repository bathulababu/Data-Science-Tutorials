...
FROM dannysdinner.menu m
JOIN dannysdinner.sales s
...
FROM (
	SELECT product_id, COUNT(1) AS most_purchased
    FROM dannysdinner.sales 
    GROUP BY product_id
...
-- Problem Statement 9 : If each $1 spent equates to 10 points and rice has a 2x points multiplier how many points would each customer have?
WITH cte_points AS (
SELECT * ,
CASE WHEN product_id = 1 THEN price * 20
	ELSE price * 10
    END AS points
FROM dannysdinner.menu)
...
LEFT JOIN dannysdinner.members me ON s.customer_id = me.customer_id;
